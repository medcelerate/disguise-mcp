#!/bin/sh
# disguise-mcp installer for macOS and Linux.
#
#   curl -fsSL https://raw.githubusercontent.com/medcelerate/disguise-mcp/main/scripts/install.sh | sh
#
# Environment overrides:
#   DISGUISEMCP_VERSION       version tag to install (default: latest release)
#   DISGUISEMCP_INSTALL_DIR   install directory (default: /usr/local/bin, else ~/.local/bin)
set -eu

REPO="medcelerate/disguise-mcp"
BIN="disguise-mcp"
...
VERSION="${DISGUISEMCP_VERSION:-}"
if [ -z "$VERSION" ]; then
  info "Resolving latest release"
  VERSION=$(curl -fsSL "https://api.github.com/repos/${REPO}/releases/latest" \
    | grep '"tag_name"' | head -1 | sed -E 's/.*"tag_name" *: *"([^"]+)".*/\1/')
  [ -n "$VERSION" ] || err "could not determine latest version; set DISGUISEMCP_VERSION"
fi
VNUM="${VERSION#v}"
...
if [ -n "${DISGUISEMCP_INSTALL_DIR:-}" ]; then
  DIR="$DISGUISEMCP_INSTALL_DIR"
...
if mv "${TMP}/${BIN}" "${DIR}/${BIN}" 2>/dev/null; then
  ...
  err "cannot write to ${DIR}; set DISGUISEMCP_INSTALL_DIR to a writable directory"
fi
